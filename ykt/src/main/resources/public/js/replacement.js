    ...
    $("#table").datagrid({
        ...
        columns: [[
           ...
           /* {field: 'phone', title: '手机号', width: 100, align: 'center'},*/
            ...
            {field: 'replacementAmount', title: '补发金额', width: 100, align: 'center',
...
$(function () {
    // 加载表格
    doQuery('/ky-ykt/personPeplacement/queryPage');
...
obj = {
    ...
    edit: function (id) {
        ...
        $("#addForm").form('clear');
        $.ajax({
            url: '/ky-ykt/personPeplacement/queryById?id=' + id,
            type: 'get',
            dataType: 'json',
            beforeSend: function () {
                $.messager.progress();
            ...
            success: function (data) {
                ...
                if (data) {
                    ...
                    //replacementAmount
                    $("#replacementAmount").val(data.replacementAmount);
                }
            },
            error: function (request) {
                $.messager.progress('close');
        ...
        $('#addForm').form('submit', {
            onSubmit: function () {
                ...
                if (lag == true) {
                    ...
                    $.ajax({
                        url: '/ky-ykt/personPeplacement/saveOrUpdate',
                        type: 'POST',
                        dataType: "json",
                        contentType: "application/json; charset=utf-8",
    ...
    //删除一个
    delOne: function (id) {
        //alert($("#table").datagrid('getSelected').id);
        id = $("#table").datagrid('getSelected').id;
        $.messager.confirm('提示信息', '是否删除所选择记录', function (flg) {
            if (flg) {
                $.ajax({
                    type: 'get',
                    url: '/ky-ykt/personPeplacement/deleteForce?id=' + id,
                    beforeSend: function () {
                        $("#table").datagrid('loading');
    ...
    upload: function () {
        $.ajax({
            type: 'post',
            url: '/ky-ykt/personPeplacement/import',
            processData: false,
            cache: false,
            contentType: false,
...
$("#personupload").click(function () {
    ...
    if (lag == true) {
        $.ajax({
            type: 'post',
            url: '/ky-ykt/personPeplacement/import?' + $("#uploadDialogForm").serialize(),
            processData: false,
            cache: false,
            contentType: false,
            data: new FormData($('#uploadForm')[0]),
...
$("#comsub").click(function () {
    ...
    $.ajax({
        ...
        dataType: "json",
        contentType: "application/json; charset=utf-8",
        url: "/ky-ykt/personPeplacement/doSubmit/" + $("#projectCombo").combobox('getValue'),
        beforeSend: function () {
            $.messager.progress();
        },
        success: function (data) {
