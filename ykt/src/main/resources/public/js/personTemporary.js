...
$(function () {
    // 加载表格
    doQuery('/ky-ykt/personTemporary/queryPage');
...
obj = {
    // 查询
    find: function () {
        doQuery('/ky-ykt/personTemporary/queryPage?' + $("#tableFindForm").serialize())
    },
    show: function (id) {
        $("#showBox").dialog({
            closed: false
        ...
        $.ajax({
            url: '/ky-ykt/personTemporary/queryById?id=' + id,
            type: 'get',
            dataType: 'json',
            beforeSend: function () {
                $.messager.progress();
    ...
    submitAudit: function (id) {
        $.messager.confirm('是否提交', '确定要提交审核吗?', function (r) {
            if (r) {
                $.ajax({
                    url: '/ky-ykt/personTemporary/doSubmitAudit',
                    type: 'post',
                    dataType: "json",
    ...
    uploadBox: function () {
        ...
        doQueryDepartment('departmentCombo');
    },
    // 编辑
    edit: function (id) {
        $("#addBox").dialog({
        ...
        $('#addForm').form('submit', {
            onSubmit: function () {
                ...
                if (lag == true) {
                    ...
                    $.ajax({
                        url: '/ky-ykt/personTemporary/saveOrUpdate',
                        type: 'POST',
                        dataType: "json",
                        contentType: "application/json; charset=utf-8",
        ...
        if (rows.length > 0) {
            $.messager.confirm('确定删除', '你确定要删除你选择的记录吗？', function (flg) {
                if (flg) {
                    ...
                    $.ajax({
                        type: 'get',
                        url: "/ky-ykt/personTemporary/deleteForce?id=" + ids.join(','),
                        beforeSend: function () {
    ...
    delOne: function (id) {
        ...
        $.messager.confirm('提示信息', '是否删除所选择记录', function (flg) {
            if (flg) {
                $.ajax({
                    type: 'get',
                    url: '/ky-ykt/personTemporary/deleteForce?id=' + id,
                    beforeSend: function () {
                        $("#table").datagrid('loading');
...
$("#upload").click(function () {
    ...
    */
    $.ajax({
        type: 'post',
        url: '/ky-ykt/personTemporary/import',
        processData: false,
        cache: false,
        contentType: false,
...
$("#personupload").click(function () {
    ...
    if (lag == true) {
        $.ajax({
            type: 'post',
            url: '/ky-ykt/personTemporary/import?' + $("#uploadDialogForm").serialize(),
            processData: false,
            cache: false,
            contentType: false,
            data: new FormData($('#uploadForm')[0]),
