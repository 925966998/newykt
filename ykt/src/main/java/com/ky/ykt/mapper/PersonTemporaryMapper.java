 ...
 */
@Mapper
public interface PersonTemporaryMapper extends BaseMapper {
     ...
     */
    @SelectProvider(type = PersonTemporarySql.class, method = "_queryPage")
    List<PersonTemporyEntity> _queryPage(Map pagerParam);

    /**
     * 根据条件查询全部 参数： map里的key为属性名（字段首字母小写） value为查询的条件，默认为等于 要改动sql请修改 *Mapper 类里的 _query() 方法
     */
    @SuppressWarnings("rawtypes")
    @SelectProvider(type = PersonTemporarySql.class, method = "_queryAll")
    List<PersonTemporyEntity> _queryAll(Map pagerParam);

    /**
     * 根据条件查询总条数 map里的key为属性名（字段首字母小写） value为查询的条件，默认为等于 要改动sql请修改 *Mapper 类里的 _query() 方法
     */
    @SelectProvider(type = PersonTemporarySql.class, method = "_queryCount")
     ...
     */
    @DeleteProvider(type = PersonTemporarySql.class, method = "_delete")
    int _delete(String id);

    /**
     * 删除（物理） 参数： id ： 要删除的记录的id
     */
    @DeleteProvider(type = PersonTemporarySql.class, method = "_deleteForce")
     ...
     */
    @SuppressWarnings("rawtypes")
    @InsertProvider(type = PersonTemporarySql.class, method = "_add")
    int _add(Map params);

    /**
     * 按实体类新增 参数： 实体类对象，必须有id属性
     */
    @InsertProvider(type = PersonTemporarySql.class, method = "_addEntity")
    int _addEntity(PersonTemporyEntity bean);

    /**
     * 更新 参数： id ： 要更新的记录的id 其他map里的参数，key为属性名（字段首字母小写） value为要更新成的值
     */
    @InsertProvider(type = PersonTemporarySql.class, method = "_update")
    int _update(Map params);

    /**
     * 按实体类更新 参数： 实体类对象，必须有id属性
     */
    @InsertProvider(type = PersonTemporarySql.class, method = "_updateEntity")
    int _updateEntity(PersonTemporyEntity bean);
}
