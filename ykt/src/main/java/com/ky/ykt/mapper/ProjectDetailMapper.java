...
public interface ProjectDetailMapper extends BaseMapper {
    ...
    @Select("select pd.projectName,SUM(p.grantAmount) AS paymentAmount,pd.totalAmount,d.departmentName,su.userName,pt.name as projectTypeName,pj.documentNum AS documentNum from project_detail pd LEFT JOIN department d on d.id=pd.operDepartment LEFT JOIN sys_user su on pd.operUser=su.id LEFT JOIN project_type pt on pt.id = pd.projectName LEFT JOIN person p on p.projectId = pd.id LEFT JOIN project pj ON pd.projectId = pj.id  where pd.id=#{id} and p.`status` != '1'")
